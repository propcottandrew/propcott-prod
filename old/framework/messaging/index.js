...
global.MessageBag = require('./lang/en');
		...
		res.render = (function(render) {
			return function() {
				if(res.locals.flash) {
					...
					for(var type in (res.locals.flash.txt||{})) {
						if(type == 'none') {
							for(var i=0; i<res.locals.flash.txt[type].length; i++)
					...
					if(res.locals.flash.bag) {
						for(var i=0; i<res.locals.flash.bag.length; i++) {
							// what if no type?
							if(util.isArray(MessageBag[res.locals.flash.bag[i]])) {
								var type = MessageBag[res.locals.flash.bag[i]][0];
